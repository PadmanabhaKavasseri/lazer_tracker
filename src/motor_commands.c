...
static CalibrationPoint calibration_points[4] = {
  {0.182, 0.441, +6, -5},    // Bottom-left
  {0.749, 0.466, -10, -6.7}, // Bottom-right
  {0.199, 0.782, +6, +6},   // Top-left
  {0.730, 0.807, -10, +6}   // Top-right
};
...
void
apply_calibration_correction(float center_x, float center_y, int *pan_angle,
  int *tilt_angle)
{
  printf("[DEBUG] Input coordinates: (%.6f, %.6f)\n", center_x, center_y);
  printf("[DEBUG] Before correction: Pan=%d, Tilt=%d\n", *pan_angle, *tilt_angle);

  // Bounding box of calibration area
  float min_x = 0.182;    // Left
  float max_x = 0.749;    // Right
  float min_y = 0.441;    // Bottom
  float max_y = 0.807;    // Top
...
int
map_to_servo_angle(float normalized_coord, int image_dimension)
{
...
int
map_to_pan_angle(float normalized_x)
{
...
int
map_to_tilt_angle(float normalized_y)
{
...
void
send_arduino_command(int pan_angle, int tilt_angle) {
  if (arduino_fd < 0)
    return;
...
void
send_arduino_command_binary(int pan_angle, int tilt_angle)
{
  if (arduino_fd < 0)
    return;
...
void
send_arduino_command_throttled(int pan_angle, int tilt_angle)
{
