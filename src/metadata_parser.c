...
DetectionResult*
parse_detection_metadata(const char *metadata_text)
{
...
DetectionResult*
parse_gst_structure(GstStructure *structure)
{
  if (!structure) {
    return NULL;
  }
  ...
  gst_structure_get_uint(structure, "sequence-num-entries",
      &result->sequence_num_entries);
  ...
  gst_structure_get_uint64(structure, "stream-timestamp",
      &result->stream_timestamp);

  // Extract bounding boxes - we know it's always GstValueArray
  const GValue *bboxes_value = gst_structure_get_value(structure,
      "bounding-boxes");
...
void
parse_detection_structure(GstStructure *det_structure, ObjectDetection *detection)
{
  if (!det_structure) {
    return;
  }
  ...
  const GValue *rectangle_value = gst_structure_get_value(det_structure,
      "rectangle");
...
void
print_detection_result(const DetectionResult *result)
{
  if (!result) return;
  ...
  if (result->num_detections == 0) {
    ...
    printf("  [%u] %s: %.1f%% at (%.6f,%.6f,%.6f,%.6f)\n",
      det->id, det->class_name ? det->class_name : "unknown", det->confidence,
          det->x, det->y, det->width, det->height);

    for (guint j = 0; j < det->num_landmarks; j++) {
      //todo printf to gprint
...
void
free_detection_result(DetectionResult *result)
{
  if (!result) return;
...
void convert_to_pixel_coordinates(const ObjectDetection *detection,
    guint image_width, guint image_height, guint *x1, guint *y1, guint *x2,
    guint *y2)
{
...
gboolean is_high_confidence_detection(const ObjectDetection *detection,
    gdouble threshold) {
