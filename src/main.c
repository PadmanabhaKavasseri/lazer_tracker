...
// Function declarations TODO where are these supposed to go?
...
static gchar*
parse_model_path(int argc, char *argv[])
{
  int opt;
  ...
  if (!model_path) {
    model_path = g_strdup("/home/ubuntu/TFLite/laser_96_large.lite");
  }
...
timing_probe_callback(GstPad *pad, GstPadProbeInfo *info, gpointer user_data)
{
  ...
  if (GST_CLOCK_TIME_IS_VALID(pts) &&
      timing->last_buffer_pts != GST_CLOCK_TIME_NONE) {
    // Calculate time since this buffer was at the previous stage
    struct timeval now;
    gettimeofday(&now, NULL);
    // decode, preproc, inference, postproc
    static struct timeval stage_times[4] = {0};
...
void
add_timing_probes(GstElement *pipeline)
{
    ...
    gst_pad_add_probe(preproc_sink, GST_PAD_PROBE_TYPE_BUFFER,
        timing_probe_callback, &decode_timing, NULL);
    gst_pad_add_probe(preproc_src, GST_PAD_PROBE_TYPE_BUFFER,
        timing_probe_callback, &preproc_timing, NULL);
    gst_pad_add_probe(inference_src, GST_PAD_PROBE_TYPE_BUFFER,
        timing_probe_callback, &inference_timing, NULL);
    gst_pad_add_probe(postproc_src, GST_PAD_PROBE_TYPE_BUFFER,
        timing_probe_callback, &postproc_timing, NULL);
}
...
main(int argc, char *argv[])
{
  ...
  g_signal_connect(metadata_sink, "new-sample",
    G_CALLBACK(on_new_metadata_sample), NULL);
...
on_new_metadata_sample(GstElement *sink, G_GNUC_UNUSED gpointer user_data)
{
  GstSample *sample;
  GstBuffer *buffer;
  GstMapInfo map;
...
void
process_metadata(char *metadata_text, size_t size)
{
  if (size > 0) {
    ...
    if (result) {
      ...
      for (guint i = 0; i < result->num_detections; i++) {
        ...
        // Filter for target with high confidence
        if (is_high_confidence_detection(det, 90.0) &&
          ...
          // Print what would be sent to Arduino//TODO to gprint
